...
export class ShoppingCartService {
    ...
    async getCartItems(cartId: string): Promise<ShoppingCartItemType[]> {
        const items = await db.select().from(shoppingCartItems).where(eq(shoppingCartItems.shoppingCartId, cartId));
    ...
    async removeItemFromCart(cartId: string, itemId: string): Promise<ShoppingCartItemType[]> {
        await db.delete(shoppingCartItems).where(
            and(eq(shoppingCartItems.shoppingCartId, cartId), eq(shoppingCartItems.id, itemId))
        );
